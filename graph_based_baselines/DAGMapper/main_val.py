...
def run_val(env,val_index):
    ...
    def update_coord(pre_coord):
        # pre_coord = pre_coord[0]
        # pre_coord[0], pre_coord[1] = pre_coord[0] * env.ROI_length, pre_coord[1] * env.ROI_width/2
        v_next = agent.train2world(pre_coord)
        if v_next is None:
            agent.taken_stop_action = 1
        else:
            v_next = [max(0,min(999,int(v_next[0]))),max(0,min(999,int(v_next[1])))]
            if (agent.v_now == v_next) or v_next is None:
                agent.taken_stop_action = 1
            agent.v_now = v_next
    ...
    for i, data in enumerate(network.dataloader_valid):
        ...
        output = sigmoid(distance_map).cpu().detach().numpy()[0,0]
        Image.fromarray(output / np.max(output) * 255).convert('RGB').save('./records/valid/seg/{}_{}_coord.png'.format(val_index,name)) 
        ...
        if len(init_points):
            for v_idx,init_point in enumerate(init_points):
                ...
                while 1:
                    ...
                    # network predictions
                    cropped_feature_tensor = agent.crop_attention_region(cat_feature_map,val_flag=True)
                    ...
                    if (agent.agent_step_counter > args.max_length * 2)\
