...
class FPN(nn.Module):
    def __init__(self, block=Bottleneck, num_blocks=[2,4,23,3],n_channels=4,n_classes=1,params=[3,1000,1000]):
        ...
        self.output_layer1 = nn.Conv2d(8, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        output1_feature = self.feature_layer1(s2 + s3 + s4 + s5)
        output1 = self._upsample(self.output_layer1(F.relu(output1_feature)), self.params[1], self.params[2])
        ...
        output2 = self._upsample(self.output_layer2(s2 + s3 + s4 + s5), self.params[1], self.params[2])
