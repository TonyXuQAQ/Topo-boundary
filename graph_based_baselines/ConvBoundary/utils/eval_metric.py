...
predicted_skel_dir = './records/test/skeleton'
predicted_graph_dir = './records/test/graph'
baseline_name = 'ConvBoundary'

def simplify_graph():
    print('Start simplifying graph...')
    with open('./dataset/data_split.json','r') as jf:
        json_data = json.load(jf)['test'][:670]
    skel_list = [x+'.png' for x in json_data]
    skel_list = os.listdir(predicted_skel_dir)
...
def thr_eval(name_in):
    ...
    img_list = os.listdir(predicted_skel_dir)
...
def APLS(name_in):
    ...
    image_list = os.listdir(predicted_skel_dir)
    image_list = [x[:-4]+'.pickle' for x in image_list]
