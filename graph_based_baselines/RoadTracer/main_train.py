...
def run_train(env,data,i):
    ...
    print('##################################################################' )
    print('RoadTracer Image: {}'.format(i))
    args = env.args
    ...
    print('Epoch: {}/{} | Image: {}/{} | Loss: coord {}/flag {}/seg {}'.format(
            env.epoch_counter,args.epochs,i,train_len,round(loss_coord_ave,3),round(loss_flag_ave,3),round(loss_seg_ave,3)))
    env.network.writer.add_scalar('Train/grow_loss',loss_coord_ave,env.training_step)
    env.network.writer.add_scalar('Train/flag_loss',loss_flag_ave,env.training_step)
