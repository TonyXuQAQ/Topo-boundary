...
def visualize():
    image_list = os.listdir('./records/refine/test/skeleton')
    for ii,image_name in enumerate(image_list):
        ...
        p = image.load()
        im = cv2.imread(os.path.join('./records/refine/test/skeleton',image_name))
        ...
        image.save(os.path.join('./records/refine/test/final_vis',image_name))
