...
from arguments import get_parser, update_dir_candidate_valid

parser = get_parser()
args = parser.parse_args()
update_dir_candidate_valid(args)

tiff_dir = args.image_dir
gt_instance_mask_dir = args.instance_mask_dir
gt_mask_dir = args.mask_dir
skeleton_dir = './records/skeleton/valid'

with open('./dataset/data_split.json','r') as jf:
    images = json.load(jf)['valid']
        ...
        if leaf_points is not None:
            ...
            while(1):
                ...
                if min_edge.pre_flag:
                    ...
                    good = np.array(good)
                    # crop a 320*320 sized patch for connection candidate validing
                    ...
                    Image.fromarray(good).convert('RGB').save('./records/candidate_valid/good/{}_{}.png'.format(image,counter))
                    Image.fromarray(cropped_tiff_save).convert('RGB').save('./records/candidate_valid/good/rgb_{}_{}.png'.format(image,counter))
            ...
            for edge in connections:
                ...
                Image.fromarray(bad).convert('RGB').save('./records/candidate_valid/bad/{}_{}.png'.format(image,bad_counter))
                Image.fromarray(cropped_tiff_save).convert('RGB').save('./records/candidate_valid/bad/rgb_{}_{}.png'.format(image,bad_counter))
...
good_counter = 0
bad_counter = 0
print('Generating connection candidates for validation image...')
with tqdm(total=len(images), unit='img') as pbar:
    for i,image in enumerate(images):
        ...
        # print('Rate: ',round(good_counter/4000*100,3),'% | Good samples: ',good_counter,'| Bad samples: ',bad_counter)
        if good_counter > 4000:
            break
